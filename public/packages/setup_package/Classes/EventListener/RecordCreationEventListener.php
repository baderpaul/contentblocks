...
namespace Contentblocks\SetupPackage\EventListener;
...
final readonly class RecordCreationEventListener
{
    ...
    public function __invoke(RecordCreationEvent $event): void
    {
        ...
        if ($rawRecord->getRecordType() !== 'contentblocks_page_list') {
            return;
        }
        if ($rawRecord->has('tx_setup_package_page') === false) {
            return;
        }
        $pages = $rawRecord->get('tx_setup_package_page');
